...
def main():
    ...
    wandb.init(
        project="ADL_PROJECT_INPAINTING",
        name=f"COPAINT_{config.dataset_name}_{config.mask_type}",

        config={
            "dataset": f"{config.dataset_name}",
            "mask_type": f"{config.mask_type}",
            "algorithm": "COPAINT",
            "sampling_algo": "o_ddim",
            "n_samples": 2
    })
    ...
    for data in tqdm(datas):
        ...
        # # wnb log
        # wandb.log({
        #     "lpips": METRICS["lpips"].report_batch(),
        #     "psnr": METRICS["psnr"].report_batch(),
        #     "ssim": METRICS["ssim"].report_batch(),
        # })
